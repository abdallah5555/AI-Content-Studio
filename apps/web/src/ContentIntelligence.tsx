...
function ResultViewer({ result, onUseIdea }: { result: IntelligenceResult | null; onUseIdea: (idea: string) => void }) {
  ...
  return (
    <section className="intel-result">
      <div className="intel-result-head">
        <div><span>الناتج</span><strong>{result.provider || 'AI'}</strong></div>
        {suggested && <button className="intel-primary small" onClick={() => onUseIdea(suggested)}>استخدم كفكرة فيديو</button>}
      </div>
      <pre>{pretty}</pre>
      {result.failover_log && result.failover_log.length > 0 && (
        <details><summary>تفاصيل التحويل بين المزودين</summary><ul>{result.failover_log.map((entry) => <li key={entry}>{entry}</li>)}</ul></details>
      )}
    </section>
  );
}
...
export function ContentIntelligence({ onBack, onUseIdea }: Props) {
  ...
  async function loadRadar(force = false) {
    ...
    try {
      const [trendItems, watched] = await Promise.all([
        force ? refreshTrends(geo, 30) : getTrends(geo, 30),
        listTrendWatchlist(),
      ]);
      setTrends(trendItems);
      setWatchlist(watched);
    } catch (requestError) {
      setError(requestError instanceof Error ? requestError.message : 'تعذر تحميل التريندات.');
    } finally { setTrendBusy(false); }
  }

  async function toggleWatch(trend: TrendItem) {
    try {
      if (watchedKeys.has(trend.key)) await unwatchTrend(trend.key);
      else await watchTrend(trend, geo);
      setWatchlist(await listTrendWatchlist());
    } catch (requestError) {
      setError(requestError instanceof Error ? requestError.message : 'تعذر تحديث قائمة المتابعة.');
    }
  }

  async function remixTrend(trend: TrendItem) {
    setSelectedTool('trend_remix');
    setToolInput(trend.title);
    setToolBusy(true); setResult(null); setError('');
    try {
      const generated = await runIntelligenceTool('trend_remix', trend.title, { context: { trend }, platform, audience });
      setResult(generated);
      setTab('tools');
    } catch (requestError) {
      setError(requestError instanceof Error ? requestError.message : 'تعذر تحليل التريند.');
    } finally { setToolBusy(false); }
  }

  async function runTool() {
    if (!toolInput.trim() && selectedTool !== 'winning_patterns') {
      setError('اكتب الموضوع أو المحتوى اللي عايز الأداة تشتغل عليه.');
      return;
    }
    setToolBusy(true); setResult(null); setError('');
    try {
      setResult(await runIntelligenceTool(selectedTool, toolInput.trim() || 'حلل نتائج المحتوى المسجلة', { platform, audience }));
    } catch (requestError) {
      setError(requestError instanceof Error ? requestError.message : 'تعذر تشغيل أداة الذكاء المحتوائي.');
    } finally { setToolBusy(false); }
  }

  async function loadInbox() {
    try { setIdeas(await listIdeaInbox()); } catch { /* page can still work */ }
  }

  async function saveIdea() {
    if (!newIdea.trim()) return;
    try {
      await addIdeaInbox(newIdea.trim());
      setNewIdea('');
      await loadInbox();
    } catch (requestError) { setError(requestError instanceof Error ? requestError.message : 'تعذر حفظ الفكرة.'); }
  }

  async function removeIdea(id: string) {
    await deleteIdeaInbox(id);
    await loadInbox();
  }

  async function loadBrand() {
    try { setBrandText(JSON.stringify(await getBrandProfile(), null, 2)); } catch { /* optional */ }
  }

  async function persistBrand() {
    setBrandBusy(true); setError('');
    try {
      const parsed = JSON.parse(brandText) as Record<string, unknown>;
      await saveBrandProfile(parsed);
    } catch (requestError) {
      setError(requestError instanceof Error ? requestError.message : 'تأكد إن Brand DNA مكتوب بصيغة JSON صحيحة.');
  ...
  async function savePerformance() {
    ...
    try {
      await recordPerformance({
        ...
        hook: performance.hook || null,
        platform,
      });
      setPerformance({ title: '', views: '', likes: '', comments: '', shares: '', hook: '' });
      setError('');
  ...
  return (
    <main className="page-shell intel-page">
      ...
      <section className="intel-hero">
        <div className="brand-badge"><Radar size={18} /> Content Intelligence Suite</div>
        <h1>اعرف تعمل إيه، إمتى، ولمين — قبل ما تبدأ الإنتاج.</h1>
        <p>رادار تريندات مجاني + فجوات محتوى + Hooks + سلاسل + تخطيط + ذاكرة تتعلم من نتائجك.</p>
      </section>
        ...
        <section className="intel-section">
          <div className="intel-toolbar">
            ...
            <div className="intel-toolbar-actions">
              ...
              <button className="intel-secondary" onClick={() => void loadRadar(true)} disabled={trendBusy}>{trendBusy ? <LoaderCircle className="spin" size={17} /> : <RefreshCw size={17} />} تحديث</button>
            </div>
          </div>
      ...
      {tab === 'tools' && (
        <section className="intel-tools-layout">
          <aside className="intel-tool-list">
            {tools.map(({ id, label, hint, icon: Icon }) => (
              <button className={selectedTool === id ? 'active' : ''} key={id} onClick={() => { setSelectedTool(id); setResult(null); }}>
                <Icon size={18} /><div><strong>{label}</strong><span>{hint}</span></div>
              </button>
            ))}
          </aside>
          <section className="intel-workbench">
            <span className="eyebrow">{selectedToolMeta.label}</span>
            <h2>{selectedToolMeta.hint}</h2>
            <textarea value={toolInput} onChange={(event) => setToolInput(event.target.value)} rows={7} placeholder="اكتب الفكرة، المجال، السكربت، التعليقات، أو المحتوى اللي عايز تحلله..." />
            <div className="intel-inline-fields">
              <input value={audience} onChange={(event) => setAudience(event.target.value)} placeholder="الجمهور: مثال شباب مصر 18–30" />
              <input value={platform} onChange={(event) => setPlatform(event.target.value)} placeholder="المنصة" />
            </div>
            <div className="intel-actions">
              <button className="intel-primary" onClick={() => void runTool()} disabled={toolBusy}>{toolBusy ? <LoaderCircle className="spin" size={17} /> : <WandSparkles size={17} />} تشغيل الأداة</button>
              {toolInput.trim() && <button className="intel-secondary" onClick={() => onUseIdea(toolInput.trim())}>ابدأ فيديو من النص الحالي</button>}
            </div>
            <ResultViewer result={result} onUseIdea={onUseIdea} />
          </section>
        </section>
      )}
      ...
      {tab === 'inbox' && (
        <section className="intel-section">
          <div className="intel-toolbar"><div><span className="eyebrow">Idea Inbox</span><h2>ارمي أي فكرة قبل ما تنساها</h2></div></div>
          <div className="idea-capture"><textarea value={newIdea} onChange={(event) => setNewIdea(event.target.value)} rows={3} placeholder="فكرة سريعة..." /><button className="intel-primary" onClick={() => void saveIdea()}><Save size={17} /> حفظ</button></div>
          <div className="idea-list">{ideas.map((idea) => <article key={idea.id}><div><strong>{idea.text}</strong><span>{new Date(idea.created_at).toLocaleString('ar-EG')}</span></div><div className="idea-actions"><button onClick={() => onUseIdea(idea.text)}>حوّل لفيديو</button><button onClick={() => { setToolInput(idea.text); setSelectedTool('idea_score'); setTab('tools'); }}>قيّمها</button><button className="danger" onClick={() => void removeIdea(idea.id)}><Trash2 size={15} /></button></div></article>)}</div>
        </section>
      )}
      ...
      {tab === 'learn' && (
        <section className="intel-tools-layout brand-layout">
          <section className="intel-workbench">
            <span className="eyebrow">Winning Pattern Memory</span><h2>سجل نتيجة كل فيديو، وسيبدأ التطبيق يتعلم إيه اللي بينجح عندك.</h2>
            <div className="performance-grid">
              <input value={performance.title} onChange={(e) => setPerformance((v) => ({ ...v, title: e.target.value }))} placeholder="اسم/موضوع الفيديو" />
              <input value={performance.views} onChange={(e) => setPerformance((v) => ({ ...v, views: e.target.value }))} placeholder="Views" inputMode="numeric" />
              <input value={performance.likes} onChange={(e) => setPerformance((v) => ({ ...v, likes: e.target.value }))} placeholder="Likes" inputMode="numeric" />
              <input value={performance.comments} onChange={(e) => setPerformance((v) => ({ ...v, comments: e.target.value }))} placeholder="Comments" inputMode="numeric" />
              <input value={performance.shares} onChange={(e) => setPerformance((v) => ({ ...v, shares: e.target.value }))} placeholder="Shares" inputMode="numeric" />
              <input value={performance.hook} onChange={(e) => setPerformance((v) => ({ ...v, hook: e.target.value }))} placeholder="الـHook المستخدم" />
            </div>
            <div className="intel-actions"><button className="intel-primary" onClick={() => void savePerformance()}><Save size={17} /> سجل النتيجة</button><button className="intel-secondary" onClick={() => { setSelectedTool('winning_patterns'); setToolInput('حلل كل نتائج المحتوى المسجلة واستخرج الأنماط الفائزة والاختبارات القادمة'); setTab('tools'); }}>حلل الأنماط الفائزة</button></div>
          </section>
        </section>
      )}
    </main>
